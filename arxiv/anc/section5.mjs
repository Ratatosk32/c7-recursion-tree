...
// The data sits in ./data next to this script. Some distributions flatten directories, so fall
// back to the script's own directory before giving up.
const load = (f) => {
  for (const dir of ['./data/', './']) {
    try { return JSON.parse(readFileSync(new URL(dir + f, import.meta.url), 'utf8')); } catch (e) { /* try the next */ }
  }
  throw new Error('cannot find ' + f + ' in ./data or alongside section5.mjs');
};
